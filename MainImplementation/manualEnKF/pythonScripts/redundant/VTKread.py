...
# Check if its the initial conditions or not
...
# print("Sampling data for " + output_filename)
...
full_cell_centers = np.array([
    np.mean([mesh.GetCell(i).GetPoints().GetPoint(j) for j in range(mesh.GetCell(i).GetNumberOfPoints())], axis=0)
    for i in range(num_cells)
])
velocity_data = vtk_to_numpy(mesh.GetCellData().GetArray("U"))  # Velocity data (e.g., "U")
Ux_data = velocity_data[:, 0]  # X-component of velocity
Uy_data = velocity_data[:, 1]  # Y-component of velocity
P_data = vtk_to_numpy(mesh.GetCellData().GetArray("p"))  # Pressure data
full_IDs = vtk_to_numpy(mesh.GetCellData().GetArray("cellID"))  # Cell IDs

if timestep != 0:
    # Extract reduced mesh data
    reduced_cell_centers = np.array([
        np.mean([mesh.GetCell(cell_id).GetPoints().GetPoint(j) for j in range(mesh.GetCell(cell_id).GetNumberOfPoints())], axis=0)
