...
# num_members = int(sys.argv[1])  # Get number of members from parent script
num_members = 8
...
# Select the relevant co-ordinates
x_p, y_p, z_p = probe_coords[p]
...
plt.legend(loc="best")
plt.tight_layout()
plt.savefig(output_path+"U_probe_series.png", dpi=300)
plt.show(block=False)
start_plot_timer = timepkg.time()
while timepkg.time() - start_plot_timer < 2:
    plt.pause(0.1)
...
plt.legend(loc="best")
plt.tight_layout()
plt.savefig(output_path+"V_probe_series.png", dpi=300)
plt.show(block=False)
start_plot_timer = timepkg.time()
while timepkg.time() - start_plot_timer < 2:
    plt.pause(0.1)
