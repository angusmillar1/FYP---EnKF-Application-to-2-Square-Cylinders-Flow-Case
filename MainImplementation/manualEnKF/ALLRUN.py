# Batch run script to run the full DNS/EnKF system
# Update the inputs at the top and ensure the relevant files are in place as per readme.md
...
# Reduce the resolution of the member results, as well as the exact solution. Write these to EnKFMeshData/ in csv format
print("\nPROCESSING FIELDS\n")
...
for sample_member in members_array: subprocess.run([sys.executable, "pythonScripts/VTKreadPV.py", sample_member, str(runtime)])
...
for loop_num in range(num_loops):
    ...
    start_EnKFloop_timing = time.time()
    ...
    for sample_member in members_array: 
        result1 = subprocess.run([sys.executable, "pythonScripts/VTKreadPV.py", sample_member, str(runtime)], capture_output=True, text=True)
        if result1.returncode != 0: break
    result2 = subprocess.run([sys.executable, "pythonScripts/calcError.py", str(start_time), str(mesh_num)])
    if result2.returncode != 0: break

    start_time += runtime
    end_EnKFloop_timing = time.time()
    print(f"\nEnKF LOOP {loop_num+2}/{num_loops+1} FINISHED\nEnKF loop {loop_num+2} elapsed time: {((end_EnKFloop_timing - start_EnKFloop_timing)/60):.2f} minutes\n")
